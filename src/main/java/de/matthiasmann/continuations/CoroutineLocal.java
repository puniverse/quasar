 ...
 * their normal counterparts in that each coroutine that accesses one (via its
 * <tt>get</tt> or <tt>set</tt> method) has its own, independently initialized
 * copy of the variable.  <tt>CoroutineLocal</tt> instances are typically private
 * static fields in classes that wish to associate state with a coroutine (e.g.,
 ...
 * <p>Each coroutine holds an implicit reference to its copy of a coroutine-local
 * variable as long as the coroutine is alive and the <tt>CoroutineLocal</tt>
 * instance is accessible; after a coroutine goes away, all of its copies of
 * coroutine-local instances are subject to garbage collection (unless other
...
public class CoroutineLocal<T> {
     ...
     * to each coroutine.  The CoroutineLocal objects act as keys,
     * searched via coroutineLocalHashCode.  This is a custom hash code
     * (useful only within CoroutineLocalMaps) that eliminates collisions
    ...
    /**
     * Sets the current coroutine's copy of this coroutine-local variable
     * to the specified value.  Most subclasses will have no need to
     * override this method, relying solely on the {@link #initialValue}
     * method to set the values of coroutine-locals.
     *
     * @param value the value to be stored in the current coroutine's copy of
     *        this coroutine-local.
     */
    ...
    /**
     * Removes the current coroutine's value for this thread-local
     * variable.  If this coroutine-local variable is subsequently
     * {@linkplain #get read} by the current thread, its value will be
     * reinitialized by invoking its {@link #initialValue} method,
     * unless its value is {@linkplain #set set} by the current coroutine
    ...
    static class CoroutineLocalMap {
         ...
         *         searching for key.
         */
        private void replaceStaleEntry(CoroutineLocal key, Object value,
                                       int staleSlot) {
            Entry[] tab = table;
            int len = tab.length;
            Entry e;
