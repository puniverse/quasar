...
import co.paralleluniverse.fibers.queues.SingleConsumerArrayObjectQueue;
import co.paralleluniverse.fibers.queues.SingleConsumerLinkedObjectQueue;
...
public class ObjectChannel<Message> extends Channel<Message> {
    ...
    public static <Message> ObjectChannel<Message> create(Fiber owner, int mailboxSize) {
        return new ObjectChannel(owner, mailboxSize > 0 ? new SingleConsumerArrayObjectQueue<Message>(mailboxSize) : new SingleConsumerLinkedObjectQueue<Message>());
    }
