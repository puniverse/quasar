 ...
 * @author pron
 */
public class ThreadObjectChannel<Message> extends ThreadChannel<Message> {
    public static <Message> ThreadObjectChannel<Message> create(Thread owner, int mailboxSize) {
        return new ThreadObjectChannel(owner, mailboxSize > 0 ? new SingleConsumerArrayObjectQueue<Message>(mailboxSize) : new SingleConsumerLinkedObjectQueue<Message>());
    }

    private ThreadObjectChannel(Thread owner, SingleConsumerQueue<Message, ?> queue) {
        super(owner, queue);
    }
     ...
     */
    public Message receive(MessageProcessor<Message> proc, long timeout, TimeUnit unit, Message currentMessage) throws InterruptedException {
    ...
    public Message receive(MessageProcessor<Message> proc, Message currentMessage) throws InterruptedException {
        return receive(proc, 0, null, currentMessage);
    }

    public Message receive(MessageProcessor<Message> proc, long timeout, TimeUnit unit) throws InterruptedException {
        return receive(proc, timeout, unit, null);
    }

    public Message receive(MessageProcessor<Message> proc) throws InterruptedException {
