    ...
    boolean exec() {
        ...
        try {
            try {
                ...
                assert task.getState() == FiberTask.RUNNABLE || task.getState() == FiberTask.LEASED;
                runningThread = null;
...
//        return map;
    ...
    private volatile long maxPark;
    private volatile long maxSchedLatency;
    ...
    private void measurePark() {
        if (timerStart == 0)
            return;
        final long time = stopTimer();
        if (time > maxPark)
            maxPark = time;
    }
    ...
    private void measureLatency() {
        if (timerStart == 0)
            return;
        final long time = stopTimer();
        if (time > maxSchedLatency)
            maxSchedLatency = time;
    }
