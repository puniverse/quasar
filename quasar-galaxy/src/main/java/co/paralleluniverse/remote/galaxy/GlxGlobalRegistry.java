    ...
    public Object register(ActorRef<?> actor) throws SuspendExecution {
        ...
        try {
            try {
                ...
                store.commit(txn);
                RemoteChannelReceiver.getReceiver(ActorUtil.getMailbox(actor), true).handleRefMessage(new GlxRemoteChannel.RefMessage(true, grid.cluster().getMyNodeId()));
    ...
    public void unregister(ActorRef<?> actor) throws SuspendExecution {
        ...
        try {
            try {
                ...
                store.commit(txn);
                RemoteChannelReceiver.getReceiver(ActorUtil.getMailbox(actor), true).handleRefMessage(new GlxRemoteChannel.RefMessage(false, grid.cluster().getMyNodeId()));
            } catch (TimeoutException e) {
