...
Reflective calls are always considered suspendable because the target method is computed at runtime so there's no way of telling if it's going to call a suspendable method or not.

Synthetic (that is, compiler-generated) [accessor methods](http://www.javaworld.com/article/2073578/java-s-synthetic-methods.html) generated by `javac` will also always be considered suspendable because it's difficult to tell at instrumentation time if they point to a suspendable method or not. This doesn't introduce any overhead though, because accessors simply "forward" calls and simple forwarders are optimized away by Quasar.

Java 8 lambdas can't declare checked exceptions, they are ultimately linked (via [`invokedynamic`](http://docs.oracle.com/javase/7/docs/technotes/guides/vm/multiple-language-support.html#invokedynamic)) to synthethic static methods that can't be annotated and it is difficult to tell at instrumentation time if they implement a suspendable interface: for these reasons at present they're always considered suspendable.
...
{:.alert .alert-info}
**Note**: only a strand, thread or fiber, can `park` itself and only another strand can `unpark` a parked strand.
...
When you run the program it just seems to get stuck. You don't know what's going exactly on but you know there might be instrumentation-related issues as you haven't yet run Quasar's diagnostic tools, so let's see how to find and fix them all.

{:.alert .alert-info}
**Note:** Quasar can potentially (at present through integration modules and starting with JDK9 fully automatically) support any JVM language because it works at the bytecode level; this is also the reason why its diagnostic messages use JVM terminology and notation rather than Java's. You'll familiarize very quickly with them, the most unusual notation being probably [JVM type signaturess](http://docs.oracle.com/javase/8/docs/technotes/guides/jni/spec/types.html); you'll start seeing some examples in the following paragraphs.
...
There's a stack frame about a strangely-named `access$100`: this is an _accessor method_ that allows the `Program.main()` method to access its static inner class' `Program.Commands.mySuspendableMethod1`. Quasar also lets us know that instrumentation has been optimized for it so that there'll be no associated overhead when running. Another strange method name is `Program.lambda$main$dedc733e$1`: it is a static method implementing the _lambda_ being used as the fiber's `SuspendableRunnable`. Since the lambda simply forwards the call to `access$100` it gets optimized too.

The verification is telling us that `mySuspendable1` is partially instrumented, and specifically the call to `myUnmarkedSuspendableMethod2` is not instrumented. Well, since `myUnmarkedSuspendableMethod2` is not marked as suspendable (and is thus also fully uninstrumented) this shouldn't come as a mystery: let's add `@Suspendable` to `myUnmarkedSuspendableMethod2`.

It tells us already that `mySuspendableMethod3` is not instrumented at all either. Why is that, considering that it calls a method of an anonymous implementation of `MyUnmarkedInterface` that seems correctly instrumented? This is because it calls it through its interface type, which is unmarked instead, so Quasar doesn't know that the call must be instrumented. In addition, there are no other suspendable calls in `mySuspendable3` so it doesn't get instrumented at all. Indeed, if we turn on the instrumentation trace by adding the `=vdc` suffix to the agent before fixing that, we see that Quasar doesn't find any suspendable calls in `mySuspendableMethod3`:
